...
use Documents\Application\DocumentCreateCommand;
...
#[Route('/document', name: 'document_create', methods: ['POST'])]
class PostDocument extends AbstractController
{
    use HandleTrait;
    ...
    public function __invoke(Request $request): JsonResponse
    {
        try {
            ...
            $document = $this->handle(new DocumentCreateCommand(
                $documentDTO['title'],
                $documentDTO['content']
            ));
        ...
        return new JsonResponse(['document'=> $document->toArray()], Response::HTTP_CREATED);
    }
}